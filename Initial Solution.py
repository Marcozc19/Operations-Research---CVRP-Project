...
allcw = pd.DataFrame(pd.read_excel(cwpair_path,sheet_name = '表2点位距离和时间表', index_col=0))
parameters=pd.DataFrame(pd.read_excel(cwpair_path,sheet_name = '表4收运车辆表')).values.tolist()
facility = pd.DataFrame(pd.read_excel(cwpair_path,sheet_name = '表3设施表', index_col=0)).values.tolist()
allcwlist = allcw.values.tolist()
allcode = pd.DataFrame(pd.read_excel(cwpair_path,sheet_name = '表1点位表', index_col=0))
...
def main_func(df1, namoinfolist,first_time,namocode,odpair,namolist,d,max_load,time_remain,work_speed,back_speed):
    print("call mainfunc")
    '''
        ...
        for j in range(len(Routes)):
            ...
            if ((len(startRoute) != 0) and (len(endRoute) != 0)):
                ...
                for i in range(len(routestore) - 1):
                    routeDistance += d[odpair.index([routestore[i], routestore[i + 1]])]  # 计算当前线路总里程
                for i in range(len(routestore)):
                    routet_stay += t[iRoutes.index([routestore[i]])]  # 计算当前线路总停留时间
    ...
    for i in range(len(Routes)):
        ...
        time = t_stay + dis / (15*1000/60) + (d[odpair.index([park, Routes[i][0]])] + d[odpair.index([Routes[i][-1], final_trans])]) / (40*1000/60)
        ...
        resultlist.append([final_trans, '转运站', '', facility[4][5], facility[4][6], 0, facility[4][4], 'all', 0, 1,
                           '3吨可通行'])  # 添加起点和终点
        resultlist.append(['点位数量', counts, '重量', weight * 0.001, '空驶行程', kdis * 0.001, '路程', distance * 0.001, '时间',
...
def output(all_code,allcode_list, statement,car_list):
    ...
    result_list = main_func(df1,namoinfolist,statement,namocodelist,odpair,namolist,d,car_list[0][1],car_list[0][2]*60,car_list[0][3]*1000/60,car_list[0][4]*1000/60)
    '''
        ...
        try:
            if len(namoinfolist)>0:
                result_list = main_func(df1, namoinfolist, statement, namocodelist, odpair, namolist, d,
                                        car_list[car_number][1], car_list[car_number][2]*60,car_list[car_number][3]*1000/60,car_list[car_number][4]*1000/60)
        except IndexError:
            pass
    ...
    if time_limit == "14:00-17:00": #分开上下午需求点
        ...
        for vehicle_limit in vehicle_limit_list:
            ...
            if vehicle_limit == 0:
                ...
                result = pd.DataFrame(output(another_df,codelist, first_time,carlist),
                                      columns=['点位编号', '点位名称', '所属路街', '经度', '纬度', '垃圾量（升/处）', '作业停留时间（分钟）', '允许时间窗口','禁止时间窗口','收运频次（次/日）', '车辆通行条件','',''])
                result.to_excel('C:/Users/zhuan/Desktop/清华大学/2022毕业论文/输出结果/output_test_下午且条件0（优化前）.xlsx')
            else:
                ...
                result = pd.DataFrame(output(another_df, codelist, first_time,carlist),
                                      columns=['点位编号', '点位名称', '所属路街', '经度', '纬度', '垃圾量（升/处）', '作业停留时间（分钟）', '允许时间窗口','禁止时间窗口','收运频次（次/日）', '车辆通行条件','',''])
                result.to_excel('C:/Users/zhuan/Desktop/清华大学/2022毕业论文/输出结果/output_test_下午且条件1（优化前）.xlsx')
    else : #上午的需求点
        vehicle_limit_list = []
        inside_rows = new_df.shape[0]
        #创建车辆通行条件
        ...
        for vehicle_limit in vehicle_limit_list:
            ...
            if vehicle_limit == 0:
                ...
                result = pd.DataFrame(output(another_df, codelist, first_time,carlist),
                                      columns=['点位编号', '点位名称', '所属路街', '经度', '纬度', '垃圾量（升/处）', '作业停留时间（分钟）', '允许时间窗口','禁止时间窗口','收运频次（次/日）', '车辆通行条件','',''])
                result.to_excel('C:/Users/zhuan/Desktop/清华大学/2022毕业论文/输出结果/output_test_上午且条件0（优化前）.xlsx')
            # 车辆通行条件=1
            else:
                ...
                result = pd.DataFrame(output(another_df, codelist, first_time,carlist),
                                      columns=['点位编号', '点位名称', '所属路街', '经度', '纬度', '垃圾量（升/处）', '作业停留时间（分钟）', '允许时间窗口','禁止时间窗口','收运频次（次/日）', '车辆通行条件','',''])
                result.to_excel('C:/Users/zhuan/Desktop/清华大学/2022毕业论文/输出结果/output_test_上午且条件1（优化前）.xlsx')
